  ...
  <div class="wrap" ref="wrap">
    <div class="content">
      <div class="podcastInfo">
        <strong class="title" v-html="setPodcastTitle()"></strong>
      ...
      <div class="listBox">
        ...
        <ul>
          <li v-for="(item,index) in search.episodeList" @click="episodeClick(item, index)" ref="episode">
            <div class="listInfo">
    ...
    <!-- 방송명 길이 계산용 더미 -->
    <span class="titleDummy" id="titleDummy">{{ search.title }}</span>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    _getEpisodeList: function (isFirst) {
      ...
      podcastApi.getEpisodeList({
        ...
        // 채널 아이디 및 타이틀 세팅
        for (let i = 0; i < window.podcastObj.search.episodeList.length; i++) {
